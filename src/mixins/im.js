  ...
  methods: {
    ...
    doSend() {
      ...
      if (receiver && message && IMSDK.isOnline()) {
        ...
        if (!receiver) {
          console.log('消息接收者是空的！')
        } else if (!message) {
          console.log('要发送的内容是空的！')
        } else if (!IMSDK.isOnline()) {
          console.log('online==false, 当前已离线，无法发送消息！')
        }
      }
    },
    onIMData(p, options) {
      ...
      if (!isSend && get(data, 'id') && get(data, 'msgType') !== msgType.isReceived.id) {
        ...
        setTimeout(() => {
          const index = get(this, 'activeItem.messages', []).findIndex((item) => item.msgBizId == data.msgBizId)
          if (index > -1) {
      ...
      if (get(this, 'activeItem.session.id') == get(data, 'sessionId')) {
        //  当前激活
        ...
        // 未激活
        const hasItem = this.Conversation.find((item) => item.session.id == data.sessionId)
        if (hasItem) {
        } else {

  ...
  <div class="table-Arccrm">
    <el-container class="h100">
        ...
        <div class="customer-page">
          <el-row>
            <div class="filter-container">
              <el-select v-model="chooseList.ticketowner" :placeholder="$t('myTicktes.Ticketowner')" clearable class="filter-select-item">
              ...
              <div style="float:right">
                ...
                <el-button v-waves type="text" style="margin: 0 24px;    margin-right: 16px;" @click="handleFilter">
                  {{ $t('table.search') }}
                </el-button>
              </div>
            </div>
          ...
          <div class="flex1" ref="tableParentBox" style="    height: 0;">
            <el-table ref="multipleTable" :max-height="tableMaxH" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;" @selection-change="handleSelectionChange">
              ...
              <el-table-column :label="$t('myTicktes.Priority')" min-width="100px" align="center" :resizable="false">
                <template slot-scope="{row}">
      ...
      <el-footer style="height:auto;">
        <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getAllTicketList" />
      </el-footer>
    </el-container>
...
<script>
import { getAllTicketList, searchTicketList } from '@/api/tickets'
...
export default {
  ...
  methods: {
    ...
    getSaleUsers() {
      var that = this
      getSaleUsers().then((json) => {
        that.ticketownerArr = json.data
        console.log(json)
      })
    },
    // 选中的时候的change事件
